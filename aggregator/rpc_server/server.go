...
func (s *RpcServer) ProcessSignedStateRootUpdateMessage(signedStateRootUpdateMessage *messages.SignedStateRootUpdateMessage, reply *bool) error {
	...
	if signedStateRootUpdateMessage.Message.Timestamp < uint64(time.Now().Unix())-60 {
		s.logger.Info("Received old message", "timestamp", signedStateRootUpdateMessage.Message.Timestamp)
		return nil
	}
...
// TODO: make this configurable and set it to a saner amount when operators have a chunking logic
const MaxCheckpointRange uint64 = 60 * 60 * 24 * 10 // 10 days
...
func (args *GetAggregatedCheckpointMessagesArgs) IsValid() error {
	...
	if (args.ToTimestamp - args.FromTimestamp) > MaxCheckpointRange {
		return errors.New("Checkpoint range exceeds max range")
	}
