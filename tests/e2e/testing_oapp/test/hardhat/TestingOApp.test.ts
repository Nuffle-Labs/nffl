...
describe('TestingOApp Test', function () {
    // Constant representing a mock Endpoint ID for testing purposes
    const eidA = 1
    const eidB = 2
    // Declaration of variables to be used in the test suite
    let TestingOApp: ContractFactory
    ...
    before(async function () {
        ...
        TestingOApp = await ethers.getContractFactory('TestingOApp')
    ...
    beforeEach(async function () {
        ...
        myOAppA = await TestingOApp.deploy(mockEndpointV2A.address, ownerA.address)
        myOAppB = await TestingOApp.deploy(mockEndpointV2B.address, ownerB.address)
